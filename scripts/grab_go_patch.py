...
CSS = """
.grab-filters{margin-bottom:10px}.grab-agent-card{margin-top:12px}.grab-table{min-width:1250px}.grab-table select{width:150px;padding:7px;border-radius:10px;border:1px solid var(--line);background:#fff}.grab-green{box-shadow:inset 5px 0 0 var(--green)}.grab-red{box-shadow:inset 5px 0 0 var(--red)}.grab-yellow{box-shadow:inset 5px 0 0 #d6a500}.grab-dot{display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:50%;background:#0f2746;color:#fff;font-weight:900}.grab-map{margin-top:10px}.grab-map iframe{width:100%;height:320px;border:0;border-radius:14px;margin-top:10px;background:#eef4fb}
"""
...
function openGrabMap(q){const f=document.getElementById('grabMapFrame'),l=document.getElementById('grabMapLabel');if(!f)return;f.src='https://maps.google.com/maps?q='+encodeURIComponent(q)+'&output=embed';if(l)l.textContent=q;document.getElementById('grabMapCard')?.scrollIntoView({behavior:'smooth',block:'center'});}
...
function renderGrabMap(rows){if(!rows.length)return '<div class="empty">Nessun punto vendita da mostrare in mappa con i filtri scelti.</div>';const q=grabQuery(rows[0]);return `<div id="grabMapCard" class="card grab-map"><div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap"><div><h3 style="text-align:left;margin-bottom:4px">Mappa</h3><div class="small-muted">Mostra il PDV selezionato dalla lista. Questa è la versione precedente stabile.</div><div class="small-muted" style="margin-top:4px"><b>Ora in mappa:</b> <span id="grabMapLabel">${esc(q)}</span></div></div><a class="btn light" target="_blank" href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(q)}">Apri Google Maps</a></div><iframe id="grabMapFrame" loading="lazy" title="Mappa Grab & Go" src="https://maps.google.com/maps?q=${encodeURIComponent(q)}&output=embed"></iframe></div>`;}
