...
<body>
  ...
  <main class="wrap">
    ...
    <section class="card">
      ...
      <div class="muted" style="margin-top:8px">Sabato, domenica e festività nazionali escluse. Durata stimata: Telepass Point 45 min, Grab & Go 15 min.</div>
    </section>
...
function iso(d){return d.toISOString().slice(0,10);}function dateLabel(d){return d.toLocaleDateString('it-IT',{weekday:'short',day:'2-digit',month:'2-digit',year:'numeric'});}
...
function visitMin(p){return p.is_grab?15:45;}function travelMin(k){return Math.round((k/55)*60+8);}function minText(n){const h=Math.floor(n/60),m=n%60;return h+'h '+String(m).padStart(2,'0');}
...
function assign(ordered,days,start){let di=0,last=start,mins=0,count=0,out=[];for(const p of ordered){const k=km(last,p),add=travelMin(k)+visitMin(p);const newDay=count>0&&((mins+add>540&&count>=4)||(count>=6&&k>8)||count>=8);if(newDay&&di<days.length-1){di++;mins=0;count=0;last=start;}const day=days[Math.min(di,days.length-1)]||new Date();const st=9*60+mins+travelMin(km(last,p));out.push(Object.assign({},p,{date:iso(day),dateLabel:dateLabel(day),time:String(Math.floor(st/60)).padStart(2,'0')+':'+String(st%60).padStart(2,'0'),travel_km:Math.round(k),visit_min:visitMin(p),day_load:minText(mins+add)}));mins+=add;count++;last=p;}return out;}
function generatePlanning(){const agent=document.getElementById('agent').value,mv=document.getElementById('month').value,inc=document.getElementById('grab').value==='yes';if(!agent||!mv){alert('Scegli agente e mese');return;}let pts=allPoints().filter(p=>p.agent===agent&&(inc||!p.is_grab)&&p.lat!=null&&p.lng!=null);if(!pts.length){alert('Nessun PV con coordinate per questo agente');return;}const days=workdays(mv),start=startPoint(pts,document.getElementById('start').value);PLAN=assign(order(pts,start,mv),days,start);renderTable(days.length);}
function renderTable(workdayCount){const days=[...new Set(PLAN.map(p=>p.date))].length,grab=PLAN.filter(p=>p.is_grab).length;const rows=PLAN.map((p,i)=>'<tr class="'+(p.is_grab?'grab':'tp')+'"><td><button class="btn light small" onclick="moveRow('+i+',-1)">↑</button> <button class="btn light small" onclick="moveRow('+i+',1)">↓</button> <button class="btn bad small" onclick="delRow('+i+')">x</button></td><td>'+esc(p.dateLabel)+'</td><td>'+esc(p.time)+'</td><td><b>'+esc(p.pdv)+'</b></td><td>'+(p.is_grab?'<span class="pill warn">Grab & Go</span>':'<span class="pill">Telepass Point</span>')+'</td><td class="city"><b>'+esc(p.city)+'</b><span>'+esc(p.address||'')+'</span></td><td>'+esc(p.province||'')+'</td><td>'+esc(p.region||'')+'</td><td>'+esc(p.focal||'')+'</td><td>'+esc(p.rzv||'')+'</td><td>'+esc(p.cr||'')+'</td><td class="num">'+fmt(p.travel_km||0)+' km</td><td class="num">'+fmt(p.visit_min||0)+' min</td><td>'+esc(p.day_load||'')+'</td></tr>').join('');document.getElementById('result').innerHTML='<section class="metric"><div class="box"><h4>PV pianificati</h4><div class="big">'+fmt(PLAN.length)+'</div></div><div class="box"><h4>Grab & Go</h4><div class="big">'+fmt(grab)+'</div></div><div class="box"><h4>Giorni usati</h4><div class="big">'+fmt(days)+'</div><div class="muted">Lavorativi: '+fmt(workdayCount||0)+'</div></div></section><section class="card"><div class="muted" style="margin-bottom:8px">Sposta le righe: date e orari vengono ricalcolati.</div><div class="table-wrap"><table><thead><tr><th>Modifica</th><th>Data</th><th>Ora</th><th>PV</th><th>Tipo</th><th>Città / indirizzo</th><th>Prov.</th><th>Regione</th><th>Focal Point ENI</th><th>RZV</th><th>CR</th><th>Km</th><th>Visita</th><th>Carico giorno</th></tr></thead><tbody>'+rows+'</tbody></table></div></section>';}
...
function exportRows(){return [['DATA','ORA','PV','TIPO','REGIONE','PROVINCIA','CITTA','INDIRIZZO','FOCAL POINT ENI','RZV','CR','KM STIMATI','DURATA VISITA','NOTE']].concat(PLAN.map(p=>[p.dateLabel,p.time,p.pdv,p.is_grab?'GRAB & GO':'TELEPASS POINT',p.region||'',p.province||'',p.city||'',p.address||'',p.focal||'',p.rzv||'',p.cr||'',p.travel_km||0,(p.visit_min||0)+' min',p.is_grab?'VISITA GRAB & GO':'']));}
...
(function init(){const now=new Date();document.getElementById('month').value=now.getFullYear()+'-'+String(now.getMonth()+1).padStart(2,'0');renderAll();loadRemote(true);})();
</script>
</body>
</html>'''
