...
CLOUD_JS_TEMPLATE = r'''
<script>
window.PLANNING_API_URL = "__API_URL__";
...
function decoratePlanningRemote() {
  ...
  box.innerHTML = '<div class="section-title">Modifiche definitive Cloudflare</div>'
    + '<div class="small-muted">API collegata: ' + window.PLANNING_API_URL
    + '<br>Modifiche definitive caricate: <b>' + ((window.PLAN_REMOTE_MODS || []).length)
    + '</b>. Aggiunte/esclusioni vengono salvate per tutti.</div>'
...
CLOUD_JS = CLOUD_JS_TEMPLATE.replace("__API_URL__", API_URL)
...
def patch_html(html):
    if "PLANNING_API_URL" in html:
        return html
    html = html.replace(
        "Queste modifiche restano salvate nel browser di chi usa il sito. Non cambiano i file GitHub.",
        "Queste modifiche vengono salvate definitivamente su Cloudflare e valgono per tutti.",
    )
    if "</body>" in html:
